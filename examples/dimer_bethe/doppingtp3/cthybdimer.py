...
def paramagnetic_hf_clean(G_iw, u_int, tp):
    ...
    g_avg = 0.5 * (G_iw['sym_up'].data + G_iw['sym_dw'].data)
    G_iw['sym_up'].data[:] = g_avg
    G_iw['sym_dw'] << G_iw['sym_up']

    g_avg = 0.5 * (G_iw['asym_up'].data + G_iw['asym_dw'].data)
    G_iw['asym_up'].data[:] = g_avg
...
def dmft_loop(setup, u_int, G_iw):
    ...
    try:
        with HDFArchive(setup['ofile'].format(**setup), 'r') as outp:
            ...
            oplot(g_iw_seed)
            plt.savefig('itgpseed.png')
            paramagnetic_hf_clean(g_iw_seed, u_int, setup['tp'])
            oplot(g_iw_seed)
            plt.savefig('itgseed.png')
            plt.close('all')
    ...
    for loop in range(last_loop, last_loop + setup['Niter']):
        if mpi.is_master_node():
            print('\n\n in loop \n', '=' * 40, loop)
            print('B{BETA} tp{tp} mu{mu}'.format(**setup), 'U' + str(u_int))

        oplot(imp_sol.G_iw)
        plt.savefig('it{}gpre.png'.format(loop))
        plt.close('all')

        paramagnetic_hf_clean(imp_sol.G_iw, u_int, setup['tp'])
        oplot(imp_sol.G_iw)
        plt.savefig('it{}gpos.png'.format(loop))
        plt.close('all')

        for name_n, g0block in imp_sol.G0_iw:
            shift = 1. if 'asym' in name_n else -1
            g0block << inverse(iOmega_n + setup['mu'] + u_int / 2. +
                               shift * setup['tp'] -
                               0.25 * imp_sol.G_iw[name_n])

        oplot(imp_sol.G0_iw)
        plt.savefig('it{}g0pos.png'.format(loop))
        plt.close('all')
...
def do_setup():
    ...
    parser.add_argument('-mu', default=0.0, type=float,
                        help='Chemical potential away Half_fill')
    parser.add_argument('-ofile', default='DIMER_{simt}_B{BETA}_tp{tp}_mu{mu}.h5',
