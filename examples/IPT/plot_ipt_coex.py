...
for beta in [16, 25, 50, 100, 200]:
    ...
    # Double occupation
    D=[2/u*(2*(0.5*s*g+u**2/8./w_n**2).real.sum()/beta -0.25*beta*u**2/8.) for g, s, u in zip(g_log, sigma_log, U)]
    axd.plot(U, np.asarray(D)+0.25, '-', label='$\\beta={}$'.format(beta))
    ## Energies
...
axd.legend(loc=0)
axd.set_ylabel(r'$\langle d \rangle$')
axd.set_xlabel('U/D')
