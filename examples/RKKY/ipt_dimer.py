...
def loop_u_tp(urange, tprange, BETA, filestr, metal=True):
    tau, w_n = gf.tau_wn_setup(dict(BETA=BETA, N_MATSUBARA=max(5*BETA, 256)))
    for u_int in urange:
...
#
    ...
    jobs = [(ur, tpr[::-1], BETA, 'disk/Dimer_rtp_ipt_B{}.h5') for BETA in BETARANGE]
    Parallel(n_jobs=-1, verbose=5)(delayed(loop_u_tp)(*job) for job in jobs)
