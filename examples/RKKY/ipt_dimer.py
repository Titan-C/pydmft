...
def loop_tp_u(tprange, u_range, beta, filestr, seed='mott gap'):
    ...
    setup = {'beta': beta, 'tprange': tprange.tolist(), 'u_range': u_range.tolist()}
...
###############################################################################
    ...
    for tp, u_int in zip(tprange, u_range):
        giw_d, giw_o, loops = rt.ipt_dmft_loop(beta, u_int, tp, giw_d, giw_o, 1e-5)
...
if __name__ == "__main__":
    ...
    BETARANGE = np.round(np.hstack(([756., 512.], np.logspace(8, -4.5, 45, base=2), [.03, .02])), 2)
